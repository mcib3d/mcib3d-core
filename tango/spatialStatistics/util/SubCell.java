...
public class SubCell {
    ...
    public Point3D draw(float distance, Distance[] distanceArray) {
        ...
        if (upperBound - lowerBound > 0) {
            i += cp.randomGenerator.nextInt(upperBound - lowerBound + 1);
        }
    ...
    private int drawBound(float distance, boolean lower, Distance[] distanceArray) {
        ...
        if (idx < 0) {
            idx = -idx - 1;
            if (idx > 0 && (idx == distanceArray.length || (distanceArray[idx].distance - distance > distance - distanceArray[idx - 1].distance))) {
                idx--;
            }
            return idx;
        } else {
            ...
            if (lower) {
                int lowerBound = idx;
                while (idx > 0 && distanceArray[lowerBound].distance == distanceArray[idx].distance) {
                    lowerBound--;
                }
                return lowerBound;
            } else {
                int upperBound = idx;
                while (upperBound < distanceArray.length && distanceArray[upperBound].distance == distanceArray[idx].distance) {
                    upperBound++;
                }
                return upperBound;
            }
